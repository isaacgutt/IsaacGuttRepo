...
public class CountStringsTest{
    ...
    String[] small = {"1", "2", "3", "what", "here", "1", "1"};
    List<Double> sequentials = new ArrayList<>();
     String[] sar = small;


@Test
    public void test() {
    CountStringsFJ fj = null;
    sar = giantArray().split(" ");
    for(int i = 0; i < 6; i++) {
        Stopwatch stopwatch = new Stopwatch(false);
        fj = new CountStringsFJ(sar, "Obama", 10000);
        double time = stopwatch.elapsedTime();
        System.out.println(time);
    }

    Stopwatch stopwatch = new Stopwatch(false);
    CountStringsFJ fj2 = new CountStringsFJ(sar, "Obama", sar.length+1);
    double time = stopwatch.elapsedTime();
    System.out.println(time);


    assertEquals(fj.doIt(), fj2.doIt());
    sar = small;
...
public void testFork(){
    ...
    //gets the times in the sequentials list to set the ratios
    testSequential();
    String full = giantArray();
    int[] thresholds = {10, 100, 1000, 5000, 10000, 50000, 100000, 1000000, 10000000};
    String[] array;
    String s2 = s;

    for (int threshold : thresholds) {
        int sub = s2.length() / 128;
        System.out.println("Thresh " + threshold);
        int iterator = 0;
        while (sub < 320436865) {
            array = full.substring(0, sub).split(" ");
            CountStringsFJ fj;
            //fj = new CountStringsFJ(array, "Obama", threshold);
            stopwatch = new Stopwatch(false);
            fj = new CountStringsFJ(array, "Obama", threshold);
            double time = stopwatch.elapsedTime();
            //end.add(time);
            System.out.println(array.length + ", " + time + ", " + time/sequentials.get(iterator));
            sub *= 2;
            iterator++;
        }
    }
...
public void testSequential(){
    ...
    String full = giantArray();
    int sub = s.length()/128;
    s = full.substring(0, sub);
    String[] array;
    while(sub < 320436865){
        array = full.substring(0, sub).split(" ");
        stopwatch = new Stopwatch(false);
        CountStringsFJ fj = new CountStringsFJ(array, "Obama", array.length+1);
        double time = stopwatch.elapsedTime();
        sequentials.add(time);
    ...
    public String giantArray(){
    s = s+s+s+s+s+s;
    s = s+s+s+s+s+s+s+s+s+s+s;
    while(s.length() < 9613109) {
            s += (s+s+s+s+s+s+s+s+s+s+s);
        }
    s+=(s+s+s);
